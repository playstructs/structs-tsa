...
echo "TX AGENT($BASHPID): TX_ID(${PENDING_TRANSACTION_ID}) OBJECT_ID(${PENDING_OBJECT_ID}) COMMAND(${PENDING_COMMAND}) PENDING_ARGS(${PENDING_ARGS}) PENDING_FLAGS(${PENDING_FLAGS})"
...
echo "TX AGENT($BASHPID): Signing.... structsd tx structs ${PENDING_COMMAND} ${PENDING_ARGS} ${PENDING_FLAGS} --from ${PENDING_ACCOUNT_NAME} ${CLIENT_FLAGS}"
